...
def create_dp(md, site,brownfield, excel_file, config_file, mynode_id):
	"""
	...
	for tenant_id in range(1,2):
		...
		for context_id in range (1, VRF_NO+1):
			vrf_name=config.get("{}-vrf{}".format(vpod_dp,context_id), "Name")
			bd_no=int(config.get("{}-vrf{}".format(vpod_dp,context_id), "BD_NO"))	
                        shd_l3=int(config.get("{}-vrf{}".format(vpod_dp,context_id), "SHD_L3"))
			l3_sub=config.get("{}-vrf{}".format(vpod_dp,context_id), "L3_SUB")
			if l3_sub=='NONE':
				continue
			l3_ip=int(config.get("{}-vrf{}".format(vpod_dp,context_id), "L3_IP"))
			l3_vlan=config.get("{}-vrf{}".format(vpod_dp,context_id), "vlan")				
			...
			if int(brownfield) == 0 or (int(brownfield) == 1 and int(mynode_id) == 301):
				...
				l3extRsNodeL3OutAtt = cobra.model.l3ext.RsNodeL3OutAtt(l3extLNodeP, tDn='topology/pod-1/node-{}'.format(L3_NODE_ID),  \
							rtrId='1.{}.{}.{}'.format(L3_NODE_ID[:1],int(L3_NODE_ID[1:3]),l3_vlan))
                                ...
                                if shd_l3==1:
				    # Exporting the contract from tenant common, required for shared l3 out
                                    vzCPIf = cobra.model.vz.CPIf(fvTenant, name='vpods-shd-bb-l3out')
				    vzRsIf = cobra.model.vz.RsIf(vzCPIf, tDn='uni/tn-common/brc-vpods-shd-bb-l3out')

                                    adv_no=int(config.get("{}-vrf{}".format(vpod_dp,context_id), "ADV_SUBNET_NO"))
                                    for adv_subnet in range (1, adv_no+1):
                                        subnet=config.get("{}-vrf{}".format(vpod_dp,context_id), "ADV_SUBNET{}".format(adv_subnet))
                                        l3extSubnet = cobra.model.l3ext.Subnet(l3extInstP, aggregate='', ip='{}'.format(subnet), name='', descr='')
                                        ipRouteP = cobra.model.ip.RouteP(l3extRsNodeL3OutAtt, aggregate='no', ip='{}'.format(subnet), pref='1', name='', descr='')
                                        ipNexthopP = cobra.model.ip.NexthopP(ipRouteP, name='', descr='', nhAddr='{}{}'.format(l3_sub, l3_ip+3))
                                        ipRouteP2 = cobra.model.ip.RouteP(l3extRsNodeL3OutAtt2, aggregate='no', ip='{}'.format(subnet), pref='1', name='', descr='')
			...
			for bd_id in range (1, bd_no+1):
			    epg_no=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_EPG_NO".format(bd_id))
			    for epg_id in range(1, 1+int(epg_no)):
				bd_name=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_Name".format(bd_id))
				epg_name=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_EPG{}_Name".format(bd_id,epg_id))
				vlan_id=int(config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_EPG{}_VLAN_NO".format(bd_id,epg_id)))
				subnet_no=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_SUBNET_NO".format(bd_id))
				...
				if int(subnet_no) != 0 and int(brownfield) != 1:
					for subnet_id in range (1,1+int(subnet_no)):
						subnet=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_GW_IP{}".format(bd_id,subnet_id))
						...
						if shd_l3==1:
                                                    fvSubnet = cobra.model.fv.Subnet(fvBD, scope='public,shared', ip=subnet) 
						else:
				...
				fvRsProv = cobra.model.fv.RsProv(fvAEPg, tnVzBrCPName='{}-epgs-to-fw-in-extEpg'.format(vrf_name), matchT='AtleastOne')
                                ...
                                if shd_l3==1:
                                      # consuming the contract interface exported from tenant common
                                      fvRsConsIf = cobra.model.fv.RsConsIf(fvAEPg, tnVzCPIfName='vpods-shd-bb-l3out')
...
def create_common_dp(md, site,brownfield, excel_file, config_file, mynode_id):
...
def config_l3ext(config_file,l3extInstP,site,l3out_name,context_id,l3extRsNodeL3OutAtt=None,l3extRsNodeL3OutAtt2=None):
	...
	bd_no=int(config.get("{}-vrf{}".format(vpod_dp,context_id), "BD_NO"))
        for bd_id in range (1, bd_no+1):
        	bd_name=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_Name".format(bd_id))
                #epg_name=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_EPG_Name".format(bd_id))
                subnet_no=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_SUBNET_NO".format(bd_id))

		bb_l3out_name=config.get("L3_OUT", "BB_L3OUT")
                for subnet_id in range (1,1+int(subnet_no)):
                    adv_check = int(config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_ADV_CHK_SUBNET{}".format(bd_id,subnet_id)))
                    if adv_check==1:
                    	subnet=config.get("{}-vrf{}".format(vpod_dp,context_id), "BD{}_ADV_SUBNET{}".format(bd_id,subnet_id))
...
def delete_common_dp(md, config_file):
...
def main():
	...
	if (int(args.node))%2==1: 
		if int(args.create[:1])==1:
			create_tenants_main_ = create_dp(session_token,args.site,args.brownfield, args.excel, args.ini, args.node)
			# configure subnet and static route in common tenant L3Out
			create_common_dp(session_token,args.site,args.brownfield, args.excel, args.ini, args.node)
		elif int(args.create[:1])==0:
			create_tenants_main_ = delete_dp(session_token,args.site)
			#delete_common_dp(session_token,args.ini)
